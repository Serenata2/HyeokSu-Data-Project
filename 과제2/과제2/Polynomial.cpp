...
{ 
	// Add함수를 쓰기전에 A, B를 정리 및 정렬 해줘야 한다.

	int aPos = 0, bPos = 0; // A와 B의 탐색해야하는 항을 나타내는 변수
	...
	for (; aPos < A.terms; aPos++) // A의 나머지 항들을 추가한다.(존재한다면)
		C.NewTerm(A.termArray[aPos].coef, A.termArray[aPos].exp);

	for (; bPos < B.terms; bPos++) // B의 나머지 항들을 추가한다.(존재한다면)
...
{
	// Mul 함수를 쓰기전에 A, B를 정리 및 정렬 해줘야 한다.

	Polynomial temp; // 임시적으로 A와 B의 곱을 저장 할 클래스 선언

	for (int i = 0; i < A.terms; i++) { // 다항식 A의 항 하나씩 다항식 B의 모든 항과 각각 곱한다.
		temp.CopyObject(B); // temp에 다항식 B복사
		for (int j = 0; j < B.terms; j++) { // 다항식 B의 모든 항에 접근
			temp.termArray[j].exp += A.termArray[i].exp; // 두 항의 곱에서 지수, 계수를 구할 땐 지수는 지수끼리 더해주고
			temp.termArray[j].coef *= A.termArray[i].coef; // 계수는 계수끼리 곱해준다
		}
		this->Add(*this, temp); // temp는 이미 정리 및 정렬이 되어 있으므로 바로 Add함수를 써준다.
	}
}

float Polynomial::Eval(float x) // (*this)의 다항식에 x 값을 대입하면 나오는 값을 반환해 주는 함수
{
	float total = 0.0f, p = 1.0f; // total에대가 각 항에 x값을 대입했을 때 나오는 값들을 저장해 준다.
	for (int i = 0; i < terms; i++) {
		p = 1.0f; // p에 1.0 대입
		for (int j = 1; j <= termArray[i].exp; j++) // (X의 i+1번째 항의 지수 승), 다른 말로 (x ^ termArray[i].exp)을 구하는 반복문
			p *= x;
		total += (termArray[i].coef) * p; // 이후 i+1번째 항의 계수와 p를 곱한 값을 total에 저장해준다.
	}
...
{ 
	...
	if (terms == 1)                                           // 항이 하나 인 경우
		...
		while (i < terms - 1) { // 첫째 항부터 마지막 전 항까지 출력
			...
			if (termArray[i + 1].coef > 0.0f) // i+2번째 항의 부호 미리 출력
				std::cout << " + ";
			else
				std::cout << " - ";
			i++; // i 값 1 증가
		...
		else if (abs(termArray[i].coef) == 1) // 계수가 1 또는 -1인 경우
...
{ 
	...
	if (k == 0) { // 다항식이 0인 경우, 아무것도 저장되지 않은 경우
...
void Polynomial::Input_Polynomial() //  다항식을 입력하는 함수
{
	...
	do { // 2개씩 값을 입력 받아 저장하고, New_Term함수를 이용해 termArray에 저장
		...
		this->NewTerm(new_coef, new_exp); // termArray에 저장
	} while (getchar() != '\n'); // enter를 칠 때까지 반복
}

bool Polynomial::CopyObject(const Polynomial& P) // P의 다항식 내용을 복사해서 (*this)다항식에 붙여넣는 함수
{ 
	// 동적할당된 Polynomial클래스는 대입연산자을 그냥 쓸 수 없기에 따로 함수를 만들었다.
